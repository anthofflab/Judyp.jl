...
type valuefunstate
  ...
  temp_cheb_vals_duals::Array{Array{Dual{Float64},1},1}
end
...
function genvaluefunstate(n_nodes, s_min, s_max)
  vs = valuefunstate(n_nodes, s_min,
    ...
    Array(Vector{Dual{Float64}}, length(n_nodes))
    )

  for i=1:length(n_nodes)
    vs.temp_cheb_vals_float64[i] = Array(Float64, n_nodes[i])
    vs.temp_cheb_vals_duals[i] = Array(Dual{Float64}, n_nodes[i])
  end
...
function getrighttemparray(::Type{Dual{Float64}},state::valuefunstate)
