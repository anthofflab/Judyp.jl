...
type optimizestate
    ...
    temp_new_state_dual::Array{ForwardDiff.Dual{2, Float64},1}
end
...
function genoptimizestate(problem::DynProgProblem, value_fun_state)
    os = optimizestate(
        ...
        value_fun_state,
        Array(Float64, length(value_fun_state.n_nodes)),
        Array(ForwardDiff.Dual{2, Float64}, length(value_fun_state.n_nodes)))
    return os
end
...
function getrighttemparray(::Type{ForwardDiff.Dual{2, Float64}},state::optimizestate)
